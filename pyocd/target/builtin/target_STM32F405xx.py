...
class STM32F405OG(CoreSightTarget):
    ...
    def __init__(self, session):
        super(STM32F405OG, self).__init__(session, self.MEMORY_MAP)
        self._svd_location = SVDFile.from_builtin("STM32F405.svd")
...
class STM32F415OG(CoreSightTarget):
    ...
    def __init__(self, session):
        super(STM32F415OG, self).__init__(session, self.MEMORY_MAP)
        self._svd_location = SVDFile.from_builtin("STM32F405.svd")
        
    def post_connect_hook(self):
        logging.debug('stm32f415og init')
        self.write32(DBGMCU.CR, DBGMCU.CR_VALUE)
        self.write32(DBGMCU.APB1_FZ, DBGMCU.APB1_FZ_VALUE)
        self.write32(DBGMCU.APB2_FZ, DBGMCU.APB2_FZ_VALUE)