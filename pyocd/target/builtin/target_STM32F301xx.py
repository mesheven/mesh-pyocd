...
class STM32F301K8(CoreSightTarget):

    VENDOR = "STMicroelectronics"
    
    MEMORY_MAP = MemoryMap(
        FlashRegion(    start=0x08000000,  length=0x10000,      blocksize=0x800, 
                                                                is_boot_memory=True,
                                                                algo=FLASH_ALGO),
    ...
    def __init__(self, link):
        super(STM32F301K8, self).__init__(link, self.MEMORY_MAP)
        self._svd_location = SVDFile.from_builtin("STM32F301.svd")
